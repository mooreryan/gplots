...
# Like heatmap.2, but with the ability to adjust dendrogram lwd and better centering of labels in rows and columns by defualt.
heatmap.lala <- function (x,
                          ...
                          srtRow = NULL,
                          srtCol = NULL,
                          # Left-right:  left justified, top-bottom: centered
                          adjRow = c(0, 0.5),
                          # Left-right:  right justified, top-bottom: centered
                          adjCol = c(1, 0.5),
                          offsetRow = 0.5,
                          offsetCol = 0.5,
                          colRow = NULL,
...
)
{
  ...
  {
    flag <- try(
      plot.dendrogram(ddr, horiz = TRUE, axes = FALSE, yaxs = "i", leaflab = "none", edgePar = list(lwd = row_dendro_lwd))
    )
  ...
  {
    flag <- try(
      plot.dendrogram(ddc, axes = FALSE, xaxs = "i", leaflab = "none", edgePar = list(lwd = col_dendro_lwd))
    )
    if("try-error" %in% class(flag))
    {
      cond <- attr(flag, "condition")
